-- Migration: phase12_public_availability
-- Purpose: read-only public_availability RPC that computes Buenos Aires local
--          slots for a published shop service, plus the BloqueoHorario
--          both-or-neither CHECK the availability predicate relies on.
-- Dependency: REQUIRES phase11_public_service_token applied first, because the
--             RPC resolves the service by Servicio.public_service_token.
--             Rolling back phase11 while this migration is applied breaks the
--             RPC, so rollbacks must run in reverse order: phase12 first.
...
--    service_role = true and anon/authenticated = false for the availability RPC.
...
WHERE n.nspname = 'public'
  AND p.proname = 'public_availability'
ORDER BY p.proname;
