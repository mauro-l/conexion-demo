-- Migration: phase19_crear_turno_fail_closed
-- Purpose: harden guards 3.7/3.8 of public.public_crear_turno so they fail
--          closed on their own when barber schedule data is NULL
--          (defense in depth behind guard 3.5). Guard 3.7 now rejects unless
--          the weekday conjunction is positively TRUE (covers false AND NULL,
--          including arrays containing NULL elements, which 3.5 cannot see);
--          guard 3.8a explicitly rejects NULL effective hours (greatest/least
--          ignore NULL inputs in Postgres). Guard 3.8b is left as-is: NULL is
--          unreachable after the hardened 3.8a. No COALESCE to shop hours was
--          used: inventing a schedule was explicitly rejected in review.
-- Dependency: REQUIRES phase15_public_cancellation (replaces only the
--             public_crear_turno body; same 10-argument signature).
-- Safety: same signature, CREATE OR REPLACE (no DROP: arity is unchanged),
--         error codes unchanged (OUTSIDE_WORKING_HOURS on 3.7/3.8,
--         SERVICE_NOT_BOOKABLE on 3.5), grants/RLS untouched.
-- Rollback: phase19_crear_turno_fail_closed_rollback.sql restores the phase15
--           body verbatim.
...
BEGIN
  ...
  --     Phase19 fail-closed hardening (defense in depth behind 3.5): the ANY
  --     comparison is NULL-unsafe in isolation -- `v_dow = ANY (NULL)` yields
  --     NULL, an array holding a NULL element (e.g. '{NULL}') yields NULL too
  --     (3.5 only sees a non-NULL array), and `IF NULL` then skips silently.
  --     `IS NOT TRUE` rejects false AND NULL alike. Error code unchanged.
  v_dow := extract(dow FROM p_inicio)::int;
  IF (v_dow = ANY (v_rec.barbero_dias)
      AND v_dow = ANY (v_rec.barberia_dias)) IS NOT TRUE
  THEN
    RETURN jsonb_build_object(
      'error', jsonb_build_object(
        'code', 'OUTSIDE_WORKING_HOURS',
  ...
  --     Phase19 fail-closed hardening (defense in depth behind 3.5):
  --     greatest/least ignore NULL inputs in Postgres, so without an explicit
  --     NULL check a half-NULL schedule could compute a bogus window instead
  --     of failing. Error code unchanged.
  ...
  -- 3.8b Grid/range check, intentionally left as-is: after the hardened 3.8a
  --     above, v_eff_open/v_eff_close are both non-NULL here, so a NULL can no
  --     longer reach this predicate and silently skip it.
...
-- signature) must exist; a leftover 9-argument overload would make PostgREST
-- answer PGRST203.
SELECT count(*) AS overloads,
       bool_and(p.pronargs = 10) AS single_10_arg_overload
FROM pg_proc p
JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'public'
