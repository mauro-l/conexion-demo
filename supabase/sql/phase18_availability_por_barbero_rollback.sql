-- Rollback: phase18_availability_por_barbero
-- Reverses phase18_availability_por_barbero.sql only. Restores the phase12
-- body of public_availability(text, text) verbatim: slots are once again
-- computed from every active barber of the shop (the pre-fix behavior).
-- The signature is unchanged, so no GRANT/REVOKE is needed: the service_role
-- grant phase12 installed survives. Deliberately does NOT touch the
-- BloqueoHorario CHECK bloqueohorario_horas_completas; that belongs to
-- phase12_public_availability_rollback. Rollback order is the reverse of
-- apply order: run this before phase12's rollback.
