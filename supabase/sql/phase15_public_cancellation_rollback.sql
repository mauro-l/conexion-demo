-- Rollback: phase15_public_cancellation
-- Drops the management RPCs and restores the phase14 nine-argument booking RPC.
-- The token hash table and ledger link are removed only after the replacement
-- function no longer depends on them. No anon/authenticated grants are reopened.

BEGIN;

DROP FUNCTION IF EXISTS public.public_gestionar_turno(text);
DROP FUNCTION IF EXISTS public.public_cancelar_turno(text);
DROP FUNCTION IF EXISTS public.public_crear_turno(text, text, timestamp, text, text, text, text, text, text, text);

-- Restore the phase14 signature and body exactly, without phase15 token logic.
...
ALTER TABLE public."BookingIdempotency" DROP COLUMN IF EXISTS turno_id;
DROP TABLE IF EXISTS public."TurnoTokenGestion";
